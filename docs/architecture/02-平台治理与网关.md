...
> **APISIX 配置模式：standalone 声明式（无 etcd）**。网关采用 `config_provider: yaml` 的声明式模式——**配置即代码**，事实源(SoT)在 `services/gateway`（`apisix.yaml`），部署期由 `deploy/charts/gateway` 渲染为 ConfigMap 注入；**不使用 etcd**、**Admin API 动态写入关闭**（纯声明式热载，改文件即 reload）。理由：避免业务路由配置与 etcd 耦合/漂移、少运维一套有状态组件、契合 GitOps（ConfigMap + Reloader / ArgoCD）。集群化的动态 Admin API（etcd 传统模式）为可选演进项，非当前默认。
