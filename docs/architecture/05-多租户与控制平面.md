...
> M2「页面真实操作」引入第一批"可写"纵切链，落档以下不可返工决策（总纲见 `docs/milestones/M2-页面真实操作-可写纵切链.md` §2）。

- **D7 数据源凭据存储**：外部数据源连接密码经**应用层对称加密**（AES-GCM）后存 PG；**主密钥部署期注入**（env / K8s Secret，**绝不入库**）；表存 `secret_cipher` 并预留 **`secret_ref` 间接层**，后续可平滑切 ESO/Vault **不改表**。明文密码**绝不落库、不进日志/响应/异常栈**。
- **D8 连接器中立**：数据源接入走 connector-SPI，**方言驱动**；MySQL 是第一个实现，**领域模型与端点后端中立、不写死 MySQL**，新增方言只加插件（守 AD-18 计算/连接后端不返工不变量）。
- **D9 写链多租户隔离**：数据源 / 元数据资产 / 成员等可写实体**一律按 `X-Tenant-Id` 强制隔离**（写时落 `tenant_id`、读时强制过滤，承 D2 不变量）。**组织管理 = `apps/console` 内按角色门控的"租户自管理"区段**（非 admin 跨租户运营）；成员**真相（SoT）在 Keycloak Organizations**，control-plane 仅经 Admin API 编排读写，**严禁另建成员凭据实体**（守 D1）。

> **D9 落地档位说明（消歧义 · 业务表隔离的"先行级"）**：§3 表「数据」维度定档 **schema/db-per-tenant** 是**目标强档**，其档位本身是**递进**（`行级 tenant_id` → `schema/db` → `独立栈`）。**M2 首批业务表（如 `metadata_asset`）落"先行级"= 行级 `tenant_id` 列 + 按 `X-Tenant-Id` 强制过滤**——这与 D9 一致,且**前向兼容**:升 schema-per-tenant 后 `tenant_id` 行级过滤仍作**兜底**保留,不返工(R2)。
> - **何时升档**：行级→schema-per-tenant 的收敛**后置**到元模型/采集引擎落地阶段（非 M2），届时由 `data-foundation`/`governance` 复用既有 `gov_<tenant>` `search_path` 路由范式;M2 阶段**不要求**动态租户 schema。
> - **动态 schema 建表 hook 归属（升档时生效）**：`gov_<tenant>` 等租户 schema 的物理创建由 **`control-plane` provisioning 开通流程**统一负责（开通租户时 `CREATE SCHEMA` + 触发各数据面 owner 的 migration 回调），**不**由子仓首访惰性各自发明;M2 期 provisioning 仍为 stub、单 ns demo 不触发。子仓**不擅自实现** `CREATE SCHEMA`。
